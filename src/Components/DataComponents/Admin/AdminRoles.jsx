...
const AdminRoles = () => {
  ...
  const getAllStates = () => {
    axios
      .get(`${process.env.REACT_APP_BASE_URL}/collections/state`)
      .then((res) => {
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${process.env.REACT_APP_BASE_URL}/collections/role/${id}`)
          .then((res) => {
  ...
  const getAllData = () => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}/collections/role?page=${page}&limit=${limit}`
      )
      .then((res) => {
        setLoader(false);
  ...
  const handleAddData = () => {
    axios
      .post(`${process.env.REACT_APP_BASE_URL}/collections/role`, currentData)
      .then((res) => {
        getAllData();
      })
      .catch((error) => {
  ...
  const handleEditData = (id) => {
    axios
      .patch(
        `${process.env.REACT_APP_BASE_URL}/collections/role/${id}`,
        currentData
      )
      .then((res) => {
        getAllData();
                ...
                <div className=" w-[300px] md:w-[500px] lg:w-[700px] border-b border-solid border-blueGray-200 p-3 flex-auto max-h-[380px] overflow-y-auto">
                  ...
                  <div class="grid md:grid-cols-2 md:gap-6">
                    <div class="relative z-0 w-full mb-5 group">
                      ...
                      <input
                        onChange={(e) =>
                          handleFormData("parentRole", e.target.value)
                        }
                        type="text"
                        name="name"
                        id="name"
                        value={currentData?.parentRole}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5      "
                        placeholder=" "
                        required
                      />
                    ...
                    <div class="relative z-0 w-full mb-5 group">
                      ...
                      <input
                        onChange={(e) =>
                          handleFormData("featureName", e.target.value)
                        }
                        type="text"
                        name="name"
                        id="name"
                        value={currentData?.featureName}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5      "
                        placeholder=" "
                        required
                      />
