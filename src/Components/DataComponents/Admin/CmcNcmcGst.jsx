...
function CmcNcmcGst() {
  ...
  const [totalGstRecords, setTotalGstRecords] = useState(0);
  ...
  const handleSelectAll = () => {
    ...
    if (!selectAll) {
      setSelectedRows(data?.map((item) => item._id)); // Change to use data instead of filteredData
    } else {
      setSelectedRows([]);
    }
  };
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${process.env.REACT_APP_BASE_URL}/admin/gst/${id}`)
          .then((res) => {
  ...
  const handleSearch = async (pageNum = 1) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/admin/gst/searchgst?q=${searchQuery}&page=${pageNum}&limit=${limit}`
      );

      setData(response.data.records || []);
      setTotalPages(response.data.totalPages || 1);
      setTotalGstRecords(response.data.totalGstRecords || 0);
      setLoader(false);
    } catch (error) {
      console.error("Error searching GST records:", error);
      setData([]);
      setTotalPages(1);
      setTotalGstRecords(0);
      setLoader(false);
    }
  };
  const handleToggleStatus = async (id, currentStatus, gst, role) => {
    const newStatus = currentStatus === "Active" ? "Inactive" : "Active";

    try {
      const response = await axios.put(
        `${process.env.REACT_APP_BASE_URL}/admin/gst/${id}`,
        {
          gst: gst, // Include gst as required by backend
          role: role, // Include role as required by backend
          status: newStatus,
        }
      );
  ...
  const getData = (pageNum = page) => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}/admin/gst?page=${pageNum}&limit=${limit}`
      )
      .then((res) => {
        setLoader(false);
        setData(res.data.records);
        setTotalPages(res.data.totalPages);
        setTotalGstRecords(res.data.total || 0);
      })
      .catch((error) => {
        setLoader(false);
        console.log(error);
          ...
          <div className="flex justify-between items-center ">
            <div className="text-sm text-gray-600">
              {isSearchMode && searchQuery ? (
                <span>
                  Search Results:{" "}
                  <span className="font-semibold">{totalGstRecords}</span> GST
                ...
                {data && data.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan="7" className="text-center p-4">
                      {isSearchMode
                        ? `No GST records found for "${searchQuery}"`
                ...
                <div className=" w-[300px] md:w-[500px] lg:w-[700px] border-b border-solid border-blueGray-200 p-3 flex-auto max-h-[380px] overflow-y-auto">
                  <div class="grid md:grid-cols-2 md:gap-6 w-full">
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        type="text"
                        onChange={(e) => handleFormData("gst", e.target.value)}
                        id="gst"
