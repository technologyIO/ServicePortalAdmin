...
function CmcNcmcTds() {
  ...
  const handleToggleStatus = async (id, currentStatus, tds, role) => {
    const newStatus = currentStatus === "Active" ? "Inactive" : "Active";

    try {
      const response = await axios.put(
        `${process.env.REACT_APP_BASE_URL}/admin/cmc-ncmc-tds/${id}`,
        {
          tds: tds, // Include tds as required by backend
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${process.env.REACT_APP_BASE_URL}/admin/cmc-ncmc-tds/${id}`)
          .then((res) => {
                ...
                <div className=" w-[300px] md:w-[500px] lg:w-[700px] border-b border-solid border-blueGray-200 p-3 flex-auto max-h-[380px] overflow-y-auto">
                  <div class="grid md:grid-cols-2 md:gap-6 w-full">
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        type="text"
                        onChange={(e) => handleFormData("tds", e.target.value)}
                        id="tds"
