...
const AdminUserType = () => {
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(
            `${process.env.REACT_APP_BASE_URL}/collections/usertype/${id}`
          )
          .then((res) => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/collections/searchusertype?q=${searchQuery}`
      );
  ...
  const getAllData = () => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}/collections/usertype?page=${page}&limit=${limit}`
      )
      .then((res) => {
        setLoader(false);
        setData(res.data.userTypes);
  ...
  const handleAddData = () => {
    axios
      .post(
        `${process.env.REACT_APP_BASE_URL}/collections/usertype`,
        currentData
      )
      .then((res) => {
        getAllData();
            ...
            <ModalDialog size="lg" className="p-2 ">
              <div className="flex items-start justify-between p-2  px-5 border-solid border-blueGray-200 rounded-t thin-scroll">
                <h3 className="text-xl font-semibold">
                  {editModal ? "Update User Type" : "Create User Type"}
                </h3>
                <div
                  onClick={() => handleCloseModal()}
                ...
                <div className=" w-[300px] md:w-[500px] lg:w-[700px] border-b border-solid border-blueGray-200 p-3 flex-auto max-h-[380px] overflow-y-auto gap-6">
                  ...
                  <div class="mt-6">
                    ...
                    <Select
                      variant="soft"
                      className="rounded-[4px] py-2 border"
                      defaultValue={currentData?.roles || ""}
                      onChange={(e, value) => handleFormData("roles", value)}
                    >
