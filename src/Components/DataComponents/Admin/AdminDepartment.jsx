...
const AdminDepartment = () => {
  ...
  const handleToggleStatus = async (id, currentStatus, departmentName) => {
    const newStatus = currentStatus === "Active" ? "Inactive" : "Active";

    try {
      const response = await axios.patch(
        `${process.env.REACT_APP_BASE_URL}/collections/depart/${id}`,
        {
          name: departmentName, // Include name as required by backend
          status: newStatus,
        }
      );
  ...
  const [isDownloadingDepartment, setIsDownloadingDepartment] = useState(false);

  const downloadDepartmentExcel = async () => {
    setIsDownloadingDepartment(true);
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BASE_URL}/excel/departments/export-departments`,
        {
          method: "GET",
        }
      );
      ...
      if (response.ok) {
        ...
        alert("Failed to download Department Excel file");
      }
    } catch (error) {
      console.error("Error downloading file:", error);
      alert("Error downloading file");
    } finally {
      setIsDownloadingDepartment(false);
    }
  };
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${process.env.REACT_APP_BASE_URL}/collections/depart/${id}`)
          .then((res) => {
  ...
  const handleSearch = async (pageNum = 1) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/collections/searchdepartment?q=${searchQuery}&page=${pageNum}&limit=${limit}`
      );

      setData(response.data.departments || []);
      setTotalPages(response.data.totalPages || 1);
      setTotalDepartments(response.data.totalDepartments || 0);
      setLoader(false);
    } catch (error) {
  ...
  const getAllData = (pageNum = page) => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}/collections/depart?page=${pageNum}&limit=${limit}`
      )
      .then((res) => {
        setLoader(false);
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex gap-3 ">
              ...
              <button
                ...
                disabled={isDownloadingDepartment}
              >
                {isDownloadingDepartment ? (
                  <>
            ...
            <div className="text-sm text-gray-600">
              {isSearchMode && searchQuery ? (
                <span>
                  ...
                  departments found for "{searchQuery}"
                </span>
              ) : (
                <span>
                  Total Records:{" "}
