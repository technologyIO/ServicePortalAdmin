...
const AdminProductGroup = () => {
  ...
  const limit = 10;
  const [totalProductGroups, setTotalProductGroups] = useState(0);
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(
            `${process.env.REACT_APP_BASE_URL}/collections/productgroup/${id}`
          )
          .then((res) => {
  ...
  const handleSearch = async (pageNum = 1) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/collections/searchproductgroupy?q=${searchQuery}&page=${pageNum}&limit=${limit}`
      );

      setData(response.data.productGroups || []);
      setTotalPages(response.data.totalPages || 1);
      setTotalProductGroups(response.data.totalProductGroups || 0);
      setLoader(false);
    } catch (error) {
      console.error("Error searching product groups:", error);
      setData([]);
      setTotalPages(1);
      setTotalProductGroups(0);
      setLoader(false);
    }
  };

  const [isDownloadingProductGroup, setIsDownloadingProductGroup] =
    useState(false);

  const downloadProductGroupExcel = async () => {
    setIsDownloadingProductGroup(true);
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BASE_URL}/excel/productgroups/export-productgroups`,
        {
          method: "GET",
        }
      );
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `product_groups_data_${
        ...
        alert("Failed to download Product Group Excel file");
      }
    } catch (error) {
      console.error("Error downloading file:", error);
      alert("Error downloading file");
    } finally {
      setIsDownloadingProductGroup(false);
    }
  };
  const getAllData = (pageNum = page) => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}/collections/productgroup?page=${pageNum}&limit=${limit}`
      )
      .then((res) => {
        setLoader(false);
        setData(res.data.productGroups);
        setTotalPages(res.data.totalPages);
        setTotalProductGroups(res.data.totalProductGroups || 0);
      })
      .catch((error) => {
        setLoader(false);
        console.log(error);
  ...
  const handleToggleStatus = async (id, currentStatus, productGroupName) => {
    ...
    try {
      const response = await axios.patch(
        `${process.env.REACT_APP_BASE_URL}/collections/productgroup/${id}`,
        {
          name: productGroupName, // Include name as required by backend
          status: newStatus,
        }
      );
        ...
        <>
          <div className="bg-gray-100 border border-gray-200 rounded-lg p-4 shadow-sm">
            ...
            <div className="flex flex-wrap justify-end gap-3">
              ...
              <button
                onClick={downloadProductGroupExcel}
                disabled={isDownloadingProductGroup}
          ...
          <div className="flex justify-between items-center ">
            <div className="text-sm text-gray-600">
              {isSearchMode && searchQuery ? (
                <span>
                  Search Results:{" "}
                  <span className="font-semibold">{totalProductGroups}</span>{" "}
                  product groups found for "{searchQuery}"
                </span>
              ) : (
                <span>
                  Total Records:{" "}
                ...
                {data && data.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan="10" className="text-center p-4">
                      {isSearchMode
                        ? `No product groups found for "${searchQuery}"`
                ...
                <div className=" w-[300px] md:w-[500px] lg:w-[700px] border-b border-solid border-blueGray-200 p-3 flex-auto max-h-[380px] overflow-y-auto gap-6">
                  <div class="grid md:grid-cols-2 md:gap-6">
                    ...
                    <div class="relative z-0 w-full mb-5 group">
                      ...
                      <input
                        onChange={(e) =>
                          handleFormData("shortcode", e.target.value)
                        }
                        type="text"
                        value={currentData?.shortcode}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5      "
                        placeholder=" "
                        required
                      />
                    ...
                    <div class="relative z-0 w-full mb-5 group">
                      ...
                      <input
                        onChange={(e) =>
                          handleFormData("ChlNo", e.target.value)
                        }
                        type="text"
                    ...
                    <div class="relative z-0 w-full mb-5 group">
                      ...
                      <input
                        onChange={(e) =>
                          handleFormData("RevNo", e.target.value)
                        }
                        type="text"
                    ...
                    <div class="relative z-0 w-full mb-5 group">
                      ...
                      <input
                        ...
                        class="bg-gray-50 border border-gray-300 text-black text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5      "
                        placeholder=" "
                        required
                      />
                    </div>
