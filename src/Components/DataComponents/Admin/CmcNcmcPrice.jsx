...
function CmcNcmcPrice() {
  ...
  const limit = 10;
  const [totalCmcNcmcPrices, setTotalCmcNcmcPrices] = useState(0);
  ...
  const [isDownloadingCmcNcmcPrice, setIsDownloadingCmcNcmcPrice] =
    useState(false);

  const downloadCmcNcmcPriceExcel = async () => {
    setIsDownloadingCmcNcmcPrice(true);
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BASE_URL}/excel/cmcncmcprices/export-cmcncmcprices`,
        {
          method: "GET",
        }
      );
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `cmc_ncmc_prices_data_${
        ...
        alert("Failed to download CMC/NCMC Price Excel file");
      }
    } catch (error) {
      console.error("Error downloading file:", error);
      alert("Error downloading file");
    } finally {
      setIsDownloadingCmcNcmcPrice(false);
    }
  };
  ...
  const handleSelectAll = () => {
    ...
    if (!selectAll) {
      setSelectedRows(data?.map((item) => item._id)); // Change filteredData to data
    } else {
      setSelectedRows([]);
    }
  };
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${process.env.REACT_APP_BASE_URL}/admin/cmcncmcprice/${id}`)
          .then((res) => {
  ...
  const handleSearch = async (pageNum = 1) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/admin/cmcncmcprice/searchprices?q=${searchQuery}&page=${pageNum}&limit=${limit}`
      );

      setData(response.data.records || []);
      setTotalPages(response.data.totalPages || 1);
      setTotalCmcNcmcPrices(response.data.totalCmcNcmcPrices || 0);
      setLoader(false);
    } catch (error) {
      console.error("Error searching CMC/NCMC prices:", error);
      setData([]);
      setTotalPages(1);
      setTotalCmcNcmcPrices(0);
      setLoader(false);
    }
  };
  ...
  const getData = (pageNum = page) => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}/admin/cmcncmcprice?page=${pageNum}&limit=${limit}`
      )
      .then((res) => {
        setLoader(false);
        setData(res.data.records);
        setTotalPages(res.data.totalPages);
        setTotalCmcNcmcPrices(res.data.total || 0);
      })
      .catch((error) => {
        setLoader(false);
        console.log(error);
  ...
  const handleToggleStatus = async (id, currentStatus, partNumber) => {
    ...
    try {
      const response = await axios.put(
        `${process.env.REACT_APP_BASE_URL}/admin/cmcncmcprice/${id}`,
        {
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                onClick={downloadCmcNcmcPriceExcel}
                disabled={isDownloadingCmcNcmcPrice}
              >
                {isDownloadingCmcNcmcPrice ? (
                  <>
          ...
          <div className="flex justify-between items-center ">
            <div className="text-sm text-gray-600">
              {isSearchMode && searchQuery ? (
                <span>
                  Search Results:{" "}
                  <span className="font-semibold">{totalCmcNcmcPrices}</span>{" "}
                  prices found for "{searchQuery}"
                </span>
              ) : (
                <span>
                  Total Records:{" "}
                ...
                {data && data.length > 0 ? (
                  data.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="p-4 font- text-md capitalize align-middle whitespace-nowrap">
                        {item?.cmcPriceWithGst}
                      </td>
                      <td className="p-4 font- text-md capitalize align-middle whitespace-nowrap">
                        {item?.ncmcPriceWithGst}
                      </td>
                      <td className="p-4 font- text-md capitalize align-middle whitespace-nowrap">
                        {item?.serviceTax}
                      </td>
                  ...
                  <tr>
                    <td colSpan="12" className="text-center p-4">
                      {isSearchMode
                        ? `No prices found for "${searchQuery}"`
                        : "No prices found"}
                    </td>
                  </tr>
                )}
                  ...
                  <div class="grid md:grid-cols-2 md:gap-6 w-full">
                    ...
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        ...
                        id="cmcPriceWithGst"
                        value={currentData?.cmcPriceWithGst}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5      "
                      />
                    </div>
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        ...
                        id="ncmcPriceWithGst"
                        value={currentData?.ncmcPriceWithGst}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5      "
                      />
                    </div>
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        type="text"
                        onChange={(e) =>
                          handleFormData("serviceTax", e.target.value)
                        }
                        id="serviceTax"
                        value={currentData?.serviceTax}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5      "
                      />
                    </div>
                    <div className="relative  w-full mb-5 group">
                      ...
                      <textarea
                        type="text"
                        onChange={(e) =>
                          handleFormData("remarks", e.target.value)
                        }
                        id="remarks"
