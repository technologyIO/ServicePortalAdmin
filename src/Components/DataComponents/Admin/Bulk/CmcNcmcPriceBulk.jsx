...
function CmcNcmcPriceBulk({ onClose }) {
    ...
    return new Promise((resolve) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          // Check for required fields (matching backend FIELD_MAPPINGS for CMC/NCMC Price)
          ...
          const isValid = Object.keys(requiredFields).every(field => foundFields[field]);
          ...
          const missingFields = Object.entries(requiredFields)
            ...
            .map(([field]) => {
              const fieldLabels = {
                ...
                product: "Product",
                cmcpricewithgst: "CMC Price with GST",
                ncmcpricewithgst: "NCMC Price with GST"
              };
  ...
  const handleUpload = async () => {
    ...
    try {
      addLiveUpdate("Starting CMC/NCMC Price data upload...", "info");

      const abortController = new AbortController();
      const timeoutId = setTimeout(() => abortController.abort(), 600000);

      const response = await fetch(
        `${process.env.REACT_APP_BASE_URL}/bulk/cmc-ncmcprice/cmcncmc-bulk-upload`,
        {
          method: "POST",
          body: formData,
      ...
      addLiveUpdate(
        "File uploaded successfully. Processing CMC/NCMC Price records...",
        "success"
      );
              ...
              try {
                ...
                setProcessingData((prev) => {
                  ...
                  if (data.processedRecords > prev.processedRecords) {
                    ...
                    addLiveUpdate(
                      `Processed ${newlyProcessed} more CMC/NCMC Price records (${data.processedRecords}/${data.totalRecords})`,
                      "success"
                    );
                  }
                  ...
                  if (data.summary?.created > prev.summary?.created) {
                    ...
                    addLiveUpdate(
                      `Created ${newCreated} new CMC/NCMC Price records (Total: ${data.summary.created})`,
                      "info"
                    );
                  }
                  ...
                  if (data.summary?.updated > prev.summary?.updated) {
                    ...
                    addLiveUpdate(
                      `Updated ${newUpdated} existing CMC/NCMC Price records (Total: ${data.summary.updated})`,
                      "info"
                    );
                  }
                  ...
                  if (data.status === "completed") {
                    addLiveUpdate(
                      `CMC/NCMC Price bulk upload completed in ${data.duration}! Created: ${data.summary.created}, Updated: ${data.summary.updated}, Skipped: ${data.summary.skippedTotal}, Failed: ${data.summary.failed}`,
                      "success"
                    );
                    setIsProcessing(false);
                    setTimeout(() => setActiveTab("results"), 100);
                  } else if (data.status === "failed") {
                    addLiveUpdate("CMC/NCMC Price processing failed!", "error");
                    setIsProcessing(false);
                  }
  ...
  // Updated CSV template with all CMC/NCMC Price fields (empty data)
  const handleDownload = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "CMC NCMC Price Template");

    // Write and download file
    XLSX.writeFile(workbook, "cmc_ncmc_price_template.xlsx");
  };
  ...
  const getProcessingSteps = () => {
    const steps = [
      ...
      },
      {
        ...
        description:
          Object.keys(processingData.headerMapping).length > 0
            ...
            : "Detecting Part Number, Description, Product, CMC Price, NCMC Price, Service Tax, Remarks, and Status columns",
        status:
          Object.keys(processingData.headerMapping).length > 0
            ? "completed"
      ...
      {
        id: 3,
        title: "Processing CMC/NCMC Price Records",
      ...
      },
      {
        ...
        description: "Completing CMC/NCMC Price bulk upload operation",
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl max-h-[95vh] overflow-hidden flex flex-col border border-gray-200">
          ...
          <div className="flex justify-between items-center">
            <div>
              <h2 className="text-2xl font-bold flex items-center gap-3">
                ...
                Bulk CMC/NCMC Price Upload
              </h2>
              <p className="text-gray-500 mt-1">
                Import and manage CMC/NCMC Price data efficiently
              </p>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="flex m-3 justify-between items-center p-4 bg-indigo-50 rounded-xl border border-indigo-200">
          <div>
            ...
            <p className="text-sm text-indigo-700">
              Download our CSV template with Part Number, Description, Product,
              CMC Price with GST, NCMC Price with GST, Service Tax, Remarks, and Status columns
            </p>
          </div>
          <button
        ...
        <div className="p-6">
                      ...
                      {file ? (
                        ...
                        <>
                          ...
                          <p className="text-xs text-center text-indigo-600 mt-2">
                            Required columns: Part Number, Description, Product,
                            CMC Price with GST, NCMC Price with GST
                          </p>
                        </>
                      )}
                    </div>
                  ...
                  <div className="flex justify-end gap-3">
                    ...
                    >
                      ...
                      {fileValidation && fileValidation.isValid
                        ? "Upload & Process CMC/NCMC Price Data ✓"
                        : "Upload & Process CMC/NCMC Price Data"}
                    </button>
                  </div>
                </div>
          ...
          {/* Results Tab - Enhanced with 5 cards for CMC/NCMC Price */}
                            ...
                            <div className="flex-1">
                              ...
                              <div className="text-sm text-gray-600 mb-1">
                                ...
                                <div className="grid grid-cols-2 gap-2 mt-1">
                                  <div>
                                    <strong>CMC Price:</strong> ₹{item.cmcPriceWithGst || 0}
                                  </div>
                                  <div>
                                    <strong>NCMC Price:</strong> ₹{item.ncmcPriceWithGst || 0}
                                  </div>
                                </div>
                              </div>
                              {item.action && (
...
export default CmcNcmcPriceBulk;
