...
function CheckListBulk({ onClose }) {
    ...
    return new Promise((resolve) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          // **UPDATED:** Required fields for CheckList schema
          ...
          // **UPDATED:** Optional fields for CheckList
          ...
          // **UPDATED:** All 6 required fields must be present for CheckList
  ...
  const handleUpload = async () => {
    ...
    try {
      addLiveUpdate("Starting CheckList data upload...", "info");

      const abortController = new AbortController();
      const timeoutId = setTimeout(() => abortController.abort(), 600000);

      const response = await fetch(
        `${process.env.REACT_APP_BASE_URL}/bulk/checklist/checklist-bulk-upload`,
        {
          method: "POST",
          body: formData,
      ...
      addLiveUpdate(
        "File uploaded successfully. Processing CheckList records...",
        "success"
      );
              ...
              try {
                ...
                setProcessingData((prev) => {
                  ...
                  if (data.processedRecords > prev.processedRecords) {
                    ...
                    addLiveUpdate(
                      `Processed ${newlyProcessed} more CheckList records (${data.processedRecords}/${data.totalRecords})`,
                      "success"
                    );
                  }
                  ...
                  if (data.summary?.created > prev.summary?.created) {
                    ...
                    addLiveUpdate(
                      `Created ${newCreated} new CheckList records (Total: ${data.summary.created})`,
                      "info"
                    );
                  }
                  ...
                  if (data.summary?.updated > prev.summary?.updated) {
                    ...
                    addLiveUpdate(
                      `Updated ${newUpdated} existing CheckList records (Total: ${data.summary.updated})`,
                      "info"
                    );
                  }
                  ...
                  if (data.status === "completed") {
                    addLiveUpdate(
                      `CheckList bulk upload completed in ${data.duration}! Created: ${data.summary.created}, Updated: ${data.summary.updated}, Skipped: ${data.summary.skipped}, Failed: ${data.summary.failed}`,
                      "success"
                    );
                    setIsProcessing(false);
                    setTimeout(() => setActiveTab("results"), 100);
                  } else if (data.status === "failed") {
                    addLiveUpdate("CheckList processing failed!", "error");
                    setIsProcessing(false);
                  }
  ...
  // **UPDATED:** CSV template with all CheckList fields
  const handleDownload = () => {
    ...
    XLSX.writeFile(workbook, "checklist_template.xlsx");
  };
  ...
  const getProcessingSteps = () => {
    const steps = [
      ...
      },
      {
        ...
        description:
          Object.keys(processingData.headerMapping).length > 0
            ...
            : "Detecting CheckList Type, Status, Checkpoint Type, Checkpoint, Product Group, Result Type, Result, Start Voltage, and End Voltage columns",
        status:
          Object.keys(processingData.headerMapping).length > 0
            ? "completed"
      ...
      {
        id: 3,
        title: "Processing CheckList Records",
      ...
      },
      {
        ...
        description: "Completing CheckList bulk upload operation",
  ...
  // Download function add karo - Checklist ke fields ke according
  const handleDownloadErrors = (exportType = "failed") => {
    ...
    // Prepare data for Excel - Checklist fields ke according
    const excelData = dataToExport.map((item) => ({
      "Row Number": item.row || "",
      "Checklist Type": item.checklisttype || "",
      Status: item.status || "",
      "Checkpoint Type": item.checkpointtype || "",
      Checkpoint: item.checkpoint || "",
      "Product Group": item.prodgroup || item.prodGroup || "",
      Result: item.result || "",
      "Result Type": item.resulttype || "",
      "Start Voltage": item.startVoltage || "",
      "End Voltage": item.endVoltage || "",
      Action: item.action || "",
      Error: item.error || "",
    ...
    XLSX.utils.book_append_sheet(
      workbook,
      worksheet,
      "Checklist Error Records"
    );

    // Generate filename with timestamp
    const timestamp = new Date().toISOString().slice(0, 19).replace(/:/g, "-");
    const filename = `checklist_errors_${exportType}_${timestamp}.xlsx`;
          ...
          <div className="flex justify-between items-center">
            <div>
              <h2 className="text-2xl font-bold flex items-center gap-3">
                ...
                Bulk CheckList Upload
              </h2>
              <p className="text-gray-500 mt-1">
                Import and manage CheckList data efficiently
              </p>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="flex m-3 justify-between items-center p-4 bg-indigo-50 rounded-xl border border-indigo-200">
          <div>
            ...
            <p className="text-sm text-indigo-700">
              Download our CSV template with CheckList Type, Status, Checkpoint
              Type, Checkpoint, Product Group, Result Type, Result, Start
              Voltage, and End Voltage columns.
            </p>
          </div>
          <button
                      ...
                      {file ? (
                        ...
                        <>
                          ...
                          <p className="text-xs text-center text-indigo-600 mt-2">
                            Required columns: CheckList Type, Status, Checkpoint
                            Type, Checkpoint, Product Group, Result Type
                          </p>
                        </>
                      )}
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-yellow-800 mt-2">
                        {processingData.summary.skipped}
                      </p>
                    </div>
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className="text-sm font-medium text-gray-800">
                                  {item.checkpoint} | {item.checklisttype}
                                </span>
                              </div>
