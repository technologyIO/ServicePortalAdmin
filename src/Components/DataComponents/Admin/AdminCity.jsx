...
const AdminCity = () => {
  ...
  const downloadCityExcel = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BASE_URL}/excel/cities/export-cities`,
        {
          method: "GET",
        }
      );
      ...
      if (response.ok) {
        ...
        alert("Failed to download City Excel file");
      }
    } catch (error) {
      console.error("Error downloading file:", error);
      alert("Error downloading file");
    } finally {
      setIsDownloadingCity(false);
    }
  };
  ...
  const handleToggleStatus = async (id, currentStatus, cityName) => {
    ...
    try {
      const response = await axios.patch(
        ...
        {
          name: cityName, // Include name as required by backend
          status: newStatus,
        }
      );
