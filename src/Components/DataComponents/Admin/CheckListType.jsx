...
function CheckListType() {
  ...
  const handleSearch = async (pageNum = 1) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/collections/searchCheckListType?q=${searchQuery}&page=${pageNum}&limit=${limit}`
      );
  ...
  const getAllCountries = (pageNum = page) => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}/collections/CheckListType?page=${pageNum}&limit=${limit}`
      )
      .then((res) => {
        setLoader(false);
        setCountries(res.data.countries || res.data.checkListTypes || []);
