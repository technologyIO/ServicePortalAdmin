...
function PendingInstallation() {
  ...
  const [totalPendingInstallations, setTotalPendingInstallations] = useState(0);
  ...
  const [
    isDownloadingPendingInstallation,
    setIsDownloadingPendingInstallation,
  ] = useState(false);
  ...
  // Filter options
  const [availableDistChannels, setAvailableDistChannels] = useState([]);
  const [availableCustomerCities, setAvailableCustomerCities] = useState([]);
  const [availableMaterials, setAvailableMaterials] = useState([]);
  const [availableSalesDistricts, setAvailableSalesDistricts] = useState([]);
  const [availableSalesOffices, setAvailableSalesOffices] = useState([]);
  const [availableCustomerCountries, setAvailableCustomerCountries] = useState(
    []
  );
  ...
  const loadFilterOptions = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/collections/pendinginstallations/filter-options`
      );
      const options = response.data;
      setAvailableDistChannels(options.distChannels || []);
      setAvailableCustomerCities(options.customerCities || []);
      setAvailableMaterials(options.materials || []);
      setAvailableSalesDistricts(options.salesDistricts || []);
      setAvailableSalesOffices(options.salesOffices || []);
      setAvailableCustomerCountries(options.customerCountries || []);
    } catch (error) {
  ...
  const applyFilters = async (pageNum = 1) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/collections/pendinginstallations/filter?${filterParams}`
      );
      setData(response.data.pendingInstallations || []);
      setTotalPages(response.data.totalPages || 1);
      setTotalPendingInstallations(
        response.data.totalPendingInstallations || 0
      );
      setLoader(false);
    } catch (error) {
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    ...
    try {
      const response = await axios.put(
        `${process.env.REACT_APP_BASE_URL}/collections/pendinginstallations/${id}`,
        { status: newStatus }
      );
  ...
  const downloadPendingInstallationExcel = async () => {
    setIsDownloadingPendingInstallation(true);
    try {
      let url = `${process.env.REACT_APP_BASE_URL}/excel/pendinginstallations/export-pendinginstallations`;
      ...
      if (response.ok) {
        ...
        a.href = downloadUrl;
        a.download = `pending_installations_data_${
        ...
        toast.error("Failed to download Pending Installation Excel file");
      }
    } catch (error) {
      console.error("Error downloading file:", error);
      toast.error("Error downloading file");
    } finally {
      setIsDownloadingPendingInstallation(false);
    }
  };
  ...
  const handleSelectAll = () => {
    ...
    if (!selectAll) {
      setSelectedRows(data?.map((installation) => installation._id));
    } else {
      setSelectedRows([]);
    }
  };

  const handleRowSelect = (installationId) => {
    if (selectedRows.includes(installationId)) {
      setSelectedRows(selectedRows.filter((id) => id !== installationId));
    } else {
      setSelectedRows([...selectedRows, installationId]);
    }
  };
  ...
  const handleOpenModal = (installation) => {
    setCurrentData(installation);
    setEditModal(true);
    setShowModal(true);
  };
      ...
      if (result.isConfirmed) {
        axios
          ...
          .then((res) => {
            Swal.fire(
              "Deleted!",
              "Pending installation has been deleted.",
              "success"
            );
          })
          .then((res) => {
  ...
  const handleSearch = async (pageNum = 1) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/collections/pendinginstallationsearch?q=${searchQuery}&page=${pageNum}&limit=${limit}`
      );
      ...
      console.error("Error searching pending installations:", error);
      setData([]);
      setTotalPages(1);
      setTotalPendingInstallations(0);
      setLoader(false);
    }
  };
  ...
  const getData = (pageNum = page) => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}/collections/pendinginstallations?page=${pageNum}&limit=${limit}`
      )
      .then((res) => {
        setLoader(false);
        setData(res.data.pendingInstallations);
        setTotalPages(res.data.totalPages);
        setTotalPendingInstallations(res.data.totalPendingInstallations || 0);
      })
      .catch((error) => {
        setLoader(false);
        console.log(error);
  ...
  const handleBulkDelete = () => {
    if (selectedRows.length === 0) {
      toast.error("Please select pending installations to delete");
      return;
    }

    Swal.fire({
      title: "Delete Selected Pending Installations?",
      text: `You are about to delete ${selectedRows.length} pending installations permanently!`,
      icon: "warning",
      showCancelButton: true,
      ...
      if (result.isConfirmed) {
        axios
          .delete(
            `${process.env.REACT_APP_BASE_URL}/collections/pendinginstallations/bulk`,
            {
          ...
          .catch((error) => {
            ...
            Swal.fire({
              ...
              text:
                ...
                "Failed to delete pending installations",
              icon: "error",
            });
          });
      }
  ...
  const handleCreate = () => {
    axios
      .post(
        `${process.env.REACT_APP_BASE_URL}/collections/pendinginstallations`,
        currentData
      )
      .then((res) => {
        getData();
          ...
          <div className="bg-gray-100 border border-gray-200 rounded-lg p-4 shadow-sm">
            ...
            <div className="flex flex-wrap justify-between items-center gap-3 mb-4">
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={downloadPendingInstallationExcel}
                  disabled={isDownloadingPendingInstallation}
                  className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                    isDownloadingPendingInstallation
              ...
              <div className="bg-white border border-gray-200 rounded-lg p-2 shadow-sm">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      value={filters.distchnl}
                      onChange={(e) =>
                        handleFilterChange("distchnl", e.target.value)
                      }
                    ...
                    >
                      <option value="">All Dist Channels</option>
                      {availableDistChannels.map((channel, index) => (
                  ...
                  <div>
                    ...
                    <select
                      value={filters.salesdist}
                      onChange={(e) =>
                        handleFilterChange("salesdist", e.target.value)
                      }
                      className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                    >
                      <option value="">All Sales Districts</option>
                      {availableSalesDistricts.map((district, index) => (
                  ...
                  <div>
                    ...
                    <select
                      value={filters.salesoff}
                      onChange={(e) =>
                        handleFilterChange("salesoff", e.target.value)
                      }
                      className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                    >
            ...
            <div className="text-sm text-gray-600">
              {isSearchMode && searchQuery ? (
                <span>
                  ...
                  pending installations found for "{searchQuery}"
                </span>
              ) : isFilterMode ? (
                <span>
                  Filtered Results:{" "}
                ...
                {data && data.length > 0 ? (
                  data.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="p-4 font- text-md capitalize align-middle whitespace-nowrap">
                        {item?.currentcustomername1}
                      </td>
                      <td className="p-4 font- text-md capitalize align-middle whitespace-nowrap">
                        {item?.currentcustomername2}
                      </td>
                      <td className="p-4 font- text-md capitalize align-middle whitespace-nowrap">
                        {item?.currentcustomercity}
                      </td>
                      <td className="p-4 font- text-md capitalize align-middle whitespace-nowrap">
                        {item?.currentcustomerregion}
                      </td>
                      <td className="p-4 font- text-md capitalize align-middle whitespace-nowrap">
                        {item?.currentcustomerpostalcode}
                      </td>
                      <td className="p-4 font- text-md capitalize align-middle whitespace-nowrap">
                        {item?.currentcustomercountry}
                      </td>
                      <td className="p-4 font- text-md capitalize align-middle whitespace-nowrap">
                        {item?.mtl_grp4}
                      </td>
                  ...
                  <tr>
                    <td colSpan="29" className="text-center p-4">
                      {isSearchMode
                        ? `No pending installations found for "${searchQuery}"`
                  ...
                  <div class="grid md:grid-cols-2 md:gap-6 w-full">
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        ...
                        required
                        onChange={(e) =>
                          handleFormData("invoiceno", e.target.value)
                        }
                        id="name"
                    ...
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        type="text"
                        onChange={(e) =>
                          handleFormData("distchnl", e.target.value)
                        }
                        id="name"
                        value={currentData?.distchnl}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5      "
                      />
                    </div>
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        type="text"
                        onChange={(e) =>
                          handleFormData("customerid", e.target.value)
                        }
                        id="name"
                    ...
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        type="text"
                        onChange={(e) =>
                          handleFormData("customername1", e.target.value)
                        }
                        id="name"
                        value={currentData?.customername1}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5      "
                      />
                    </div>
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        type="text"
                        onChange={(e) =>
                          handleFormData("customername2", e.target.value)
                        }
                        id="name"
                        value={currentData?.customername2}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5      "
                      />
                    </div>
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        type="text"
                        onChange={(e) =>
                          handleFormData("customercity", e.target.value)
                        }
                        id="name"
                        value={currentData?.customercity}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5      "
                      />
                    </div>
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        type="text"
                        onChange={(e) =>
                          handleFormData("customerpostalcode", e.target.value)
                        }
                        id="name"
                        value={currentData?.customerpostalcode}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5      "
                      />
                    </div>
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        type="text"
                        onChange={(e) =>
                          handleFormData("material", e.target.value)
                        }
                        id="name"
                    ...
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        type="text"
                        onChange={(e) =>
                          handleFormData("serialno", e.target.value)
                        }
                        id="name"
                    ...
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        type="text"
                        onChange={(e) =>
                          handleFormData("salesdist", e.target.value)
                        }
                        id="name"
                        value={currentData?.salesdist}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5      "
                      />
                    </div>
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        type="text"
                        onChange={(e) =>
                          handleFormData("salesoff", e.target.value)
                        }
                        id="name"
                        value={currentData?.salesoff}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5      "
                      />
                    </div>
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        type="text"
                        onChange={(e) =>
                          handleFormData("customercountry", e.target.value)
                        }
                        id="name"
                        value={currentData?.customercountry}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5      "
                      />
                    </div>
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        type="text"
                        onChange={(e) =>
                          handleFormData("customerregion", e.target.value)
                        }
                        id="name"
                        value={currentData?.customerregion}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5      "
                      />
                    </div>
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        type="text"
                        onChange={(e) =>
                          handleFormData("currentcustomerid", e.target.value)
                        }
                        id="name"
                        value={currentData?.currentcustomerid}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5      "
                      />
                    </div>
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        type="text"
                        onChange={(e) =>
                          handleFormData("currentcustomername1", e.target.value)
                        }
                        id="name"
                        value={currentData?.currentcustomername1}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5      "
                      />
                    </div>
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        type="text"
                        onChange={(e) =>
                          handleFormData("currentcustomername2", e.target.value)
                        }
                        id="name"
                        value={currentData?.currentcustomername2}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5      "
                      />
                    </div>
                    <div className="relative  w-full mb-5 group">
                      ...
                      <Select
                        ...
                        defaultValue={currentData?.currentcustomercity || ""}
                        onChange={(e, value) =>
                          handleFormData("currentcustomercity", value)
                        }
                      >
                    ...
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        ...
                        id="name"
                        value={currentData?.currentcustomerregion}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5      "
                      />
                    </div>
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        ...
                        id="name"
                        value={currentData?.currentcustomerpostalcode}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5      "
                      />
                    </div>
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        ...
                        id="name"
                        value={currentData?.currentcustomercountry}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5      "
                      />
                    </div>
                    <div className="relative  w-full mb-5 group">
                      ...
                      <input
                        type="text"
                        onChange={(e) =>
                          handleFormData("mtl_grp4", e.target.value)
                        }
                        id="name"
                        value={currentData?.mtl_grp4}
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5      "
                      />
                    </div>
                    <div className="relative  w-full mb-5 group">
