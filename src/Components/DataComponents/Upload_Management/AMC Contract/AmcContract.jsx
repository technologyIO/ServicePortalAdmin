...
function AmcContract() {
  ...
  // Filter options
  const [availableSalesDocs, setAvailableSalesDocs] = useState([]);
  const [availableSaTypes, setAvailableSaTypes] = useState([]);
  ...
  const loadFilterOptions = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/collections/amccontracts/filter-options`
      );
      const options = response.data;
      setAvailableSalesDocs(options.salesDocs || []);
      setAvailableSaTypes(options.saTypes || []);
      setAvailableSerialNumbers(options.serialNumbers || []);
  ...
  const applyFilters = async (pageNum = 1) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/collections/amccontracts/filter?${filterParams}`
      );
  ...
  const downloadAMCContractExcel = async () => {
    ...
    try {
      let url = `${process.env.REACT_APP_BASE_URL}/excel/amccontracts/export-amccontracts`;
      ...
      if (response.ok) {
        ...
        toast.error("Failed to download AMC Contract Excel file");
      }
    } catch (error) {
  ...
  const handleSelectAll = () => {
    ...
    if (!selectAll) {
      setSelectedRows(data?.map((contract) => contract._id));
    } else {
      setSelectedRows([]);
    }
  };
  ...
  const handleRowSelect = (contractId) => {
    if (selectedRows.includes(contractId)) {
      setSelectedRows(selectedRows.filter((id) => id !== contractId));
    } else {
      setSelectedRows([...selectedRows, contractId]);
    }
  };
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          ...
          .then((res) => {
            Swal.fire("Deleted!", "AMC contract has been deleted.", "success");
          })
          .then((res) => {
            getData();
          })
              ...
              <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      value={filters.salesdoc}
                      onChange={(e) =>
                        handleFilterChange("salesdoc", e.target.value)
                      }
                    ...
                    >
                      <option value="">All Sales Docs</option>
                      {availableSalesDocs.map((doc, index) => (
                  ...
                  <div>
                    ...
                    <select
                      value={filters.satypeZDRC_ZDRN}
                      onChange={(e) =>
                        handleFilterChange("satypeZDRC_ZDRN", e.target.value)
                      }
                      className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                    >
                      <option value="">All SA Types</option>
                      {availableSaTypes.map((type, index) => (
