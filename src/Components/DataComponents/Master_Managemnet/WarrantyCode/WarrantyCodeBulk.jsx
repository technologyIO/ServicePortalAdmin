...
function WarrantyCodeBulk({ onClose }) {
    ...
    return new Promise((resolve) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          // Check for required fields (matching backend FIELD_MAPPINGS) - 3 REQUIRED FIELDS
          ...
          // All 3 fields are required: warrantycodeid, description, months
          const isValid =
            foundFields.warrantycodeid &&
  ...
  // Download function add karo - WarrantyCode ke fields ke according
  const handleDownloadErrors = (exportType = "failed") => {
    ...
    // Prepare data for Excel - WarrantyCode fields ke according
    const excelData = dataToExport.map((item) => ({
      "Row Number": item.row || "",
      "Warranty Code ID": item.warrantycodeid || "",
    ...
    XLSX.utils.book_append_sheet(
      workbook,
      worksheet,
      "WarrantyCode Error Records"
    );

    // Generate filename with timestamp
    const timestamp = new Date().toISOString().slice(0, 19).replace(/:/g, "-");
    const filename = `warranty_code_errors_${exportType}_${timestamp}.xlsx`;
    ...
    // Add live update
    addLiveUpdate(
      `📥 Warranty Code error data exported: ${filename}`,
      "success"
    );
  };
  ...
  const handleDownload = () => {
    ...
    const data = [
      ["Warranty Code", "Description", "Months", "Status"], // Headers with Status field added
      ["", "", "", ""], // Empty row 1
      ["", "", "", ""], // Empty row 2
      ["", "", "", ""], // Empty row 3
    ];
    ...
    // Add worksheet to workbook
    XLSX.utils.book_append_sheet(workbook, worksheet, "Warranty Code Template");

    // Write and download file
    XLSX.writeFile(workbook, "warranty_code_template.xlsx");
  };
        ...
        <div className="flex m-3 justify-between items-center p-4 bg-blue-50 rounded-xl border border-blue-200">
          <div>
            ...
            <p className="text-sm text-blue-700">
              Download our CSV template with Warranty Code, Description, and
              Months columns
            </p>
          </div>
          <button
