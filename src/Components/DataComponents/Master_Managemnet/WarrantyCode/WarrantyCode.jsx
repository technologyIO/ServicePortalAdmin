...
function WarrantyCode() {
  ...
  const [isDownloadingWarrantyCode, setIsDownloadingWarrantyCode] = useState(false);
  ...
  // Filter options
  const [availableWarrantyCodes, setAvailableWarrantyCodes] = useState([]);
  ...
  const loadFilterOptions = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/collections/warrantycode/filter-options`
      );
      const options = response.data;
      setAvailableWarrantyCodes(options.warrantyCodes || []);
  ...
  const applyFilters = async (pageNum = 1) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/collections/warrantycode/filter?${filterParams}`
      );
  ...
  const downloadWarrantyCodeExcel = async () => {
    ...
    try {
      let url = `${process.env.REACT_APP_BASE_URL}/excel/warrantycodes/export-warrantycodes`;
      ...
      if (response.ok) {
        ...
        a.href = downloadUrl;
        a.download = `warranty_codes_data_${new Date().toISOString().split("T")[0]}.xlsx`;
        ...
        toast.error("Failed to download Warranty Code Excel file");
      }
    } catch (error) {
  ...
  const handleSelectAll = () => {
    ...
    if (!selectAll) {
      setSelectedRows(data?.map((warrantyCode) => warrantyCode._id));
    } else {
      setSelectedRows([]);
    }
  };

  const handleRowSelect = (warrantyCodeId) => {
    if (selectedRows.includes(warrantyCodeId)) {
      setSelectedRows(selectedRows.filter((id) => id !== warrantyCodeId));
    } else {
      setSelectedRows([...selectedRows, warrantyCodeId]);
    }
  };
  ...
  const handleOpenModal = (warrantyCode) => {
    setCurrentData(warrantyCode);
    setEditModal(true);
    setShowModal(true);
  };
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          ...
          .then((res) => {
            Swal.fire("Deleted!", "Warranty code has been deleted.", "success");
          })
          .then((res) => {
            getData();
          })
              ...
              <div className="bg-white border border-gray-200 rounded-lg p-4 mb-4 shadow-sm">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      value={filters.warrantycodeid}
                      onChange={(e) => handleFilterChange('warrantycodeid', e.target.value)}
                    ...
                    >
                      <option value="">All Warranty Codes</option>
                      {availableWarrantyCodes.map((code, index) => (
