...
const UserData = () => {
  ...
  const applyFilters = async (pageNum = 1) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/excel/users/filter?${filterParams}`
      );
  ...
  const handleSearch = async (pageNum = 1) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/excel/users/usersearch?q=${searchQuery}&page=${pageNum}&limit=${limit}`
      );
      setData(response.data.users);
      setTotalPages(response.data.totalPages);
      setTotalUsers(response.data.totalUsers);
  ...
  const downloadExcel = async () => {
    ...
    try {
      let url = `${process.env.REACT_APP_BASE_URL}/excel/users/export-users`;
