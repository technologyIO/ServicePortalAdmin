...
function DealerBulk({ onClose }) {
    ...
    return new Promise((resolve) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          // Normalize headers for comparison (matching backend FIELD_MAPPINGS for Dealer)
          ...
          // Check for required fields (matching backend FIELD_MAPPINGS for Dealer)
  ...
  const handleUpload = async () => {
    ...
    try {
      addLiveUpdate("Starting Dealer data upload...", "info");

      const abortController = new AbortController();
      const timeoutId = setTimeout(() => abortController.abort(), 600000);

      const response = await fetch(
        `${process.env.REACT_APP_BASE_URL}/bulk/dealer/dealer-bulk-upload`,
        {
          method: "POST",
          body: formData,
      ...
      addLiveUpdate(
        "File uploaded successfully. Processing Dealer records...",
        "success"
      );
              ...
              try {
                ...
                setProcessingData((prev) => {
                  ...
                  if (data.processedRecords > prev.processedRecords) {
                    ...
                    addLiveUpdate(
                      `Processed ${newlyProcessed} more Dealer records (${data.processedRecords}/${data.totalRecords})`,
                      "success"
                    );
                  }
                  ...
                  if (data.summary?.created > prev.summary?.created) {
                    ...
                    addLiveUpdate(
                      `Created ${newCreated} new Dealer records (Total: ${data.summary.created})`,
                      "info"
                    );
                  }
                  ...
                  if (data.summary?.updated > prev.summary?.updated) {
                    ...
                    addLiveUpdate(
                      `Updated ${newUpdated} existing Dealer records (Total: ${data.summary.updated})`,
                      "info"
                    );
                  }
                  ...
                  if (data.status === "completed") {
                    addLiveUpdate(
                      `Dealer bulk upload completed in ${data.duration}! Created: ${data.summary.created}, Updated: ${data.summary.updated}, Skipped: ${data.summary.skippedTotal}, Failed: ${data.summary.failed}`,
                      "success"
                    );
                    setIsProcessing(false);
                    setTimeout(() => setActiveTab("results"), 100);
                  } else if (data.status === "failed") {
                    addLiveUpdate("Dealer processing failed!", "error");
                    setIsProcessing(false);
                  }
  ...
  // Download function add karo (same as before, but dealer ke liye adapt kiya)
  const handleDownloadErrors = (exportType = "failed") => {
    ...
    // Prepare data for Excel - Dealer fields ke according
    const excelData = dataToExport.map((item) => ({
      ...
      "Person Responsible": item.personresponsible || "",
      Status: item.status || "",
      Action: item.action || "",
      Error: item.error || "",
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Dealer Error Records");

    // Generate filename with timestamp
    const timestamp = new Date().toISOString().slice(0, 19).replace(/:/g, "-");
    const filename = `dealer_errors_${exportType}_${timestamp}.xlsx`;

    // Download file
    XLSX.writeFile(workbook, filename);
  };

  // Updated CSV template with all Dealer fields (empty data)
  const handleDownload = () => {
    ...
    // Add worksheet to workbook
    XLSX.utils.book_append_sheet(workbook, worksheet, "Dealer Template");

    // Write and download file
    XLSX.writeFile(workbook, "dealer_template.xlsx");
  };
  ...
  const getProcessingSteps = () => {
    const steps = [
      ...
      },
      {
        ...
        description:
          Object.keys(processingData.headerMapping).length > 0
            ...
            : "Detecting Dealer Name, Dealer Code, Email, State, City, Address, Pincode, Person Responsible, Employee ID, and Status columns",
        status:
          Object.keys(processingData.headerMapping).length > 0
            ? "completed"
      ...
      {
        id: 3,
        title: "Processing Dealer Records",
      ...
      },
      {
        ...
        description: "Completing Dealer bulk upload operation",
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl max-h-[95vh] overflow-hidden flex flex-col border border-gray-200">
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-500 mt-1">
                Import and manage Dealer data efficiently
              </p>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="flex m-3 justify-between items-center p-4 bg-indigo-50 rounded-xl border border-indigo-200">
          <div>
            ...
            <p className="text-sm text-indigo-700">
              Download our CSV template with Dealer Name, Dealer Code, Email,
              State, City, Address, Pincode, Person Responsible, Employee ID,
              and Status columns. Use comma-separated values for multiple
              states/cities/contacts.
            </p>
          </div>
          <button
        ...
        <div className="p-6">
                      ...
                      {file ? (
                        ...
                        <>
                          ...
                          <p className="text-xs text-center text-indigo-600 mt-2">
                            Required columns: Dealer Name, Dealer Code, Email,
          ...
          {/* Results Tab - Enhanced with 5 cards for Dealer */}
