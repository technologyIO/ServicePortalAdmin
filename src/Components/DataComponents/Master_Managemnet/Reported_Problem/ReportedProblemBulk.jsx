...
function ReportedProblemBulk({ onClose }) {
    ...
    return new Promise((resolve) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          // Check for required fields (matching backend FIELD_MAPPINGS) - 5 REQUIRED FIELDS
  ...
  // Download function add karo - ReportedProblem ke fields ke according
  const handleDownloadErrors = (exportType = "failed") => {
    ...
    // Prepare data for Excel - ReportedProblem fields ke according
    const excelData = dataToExport.map((item) => ({
      "Row Number": item.row || "",
      Catalog: item.catalog || "",
      "Code Group": item.codegroup || "",
      "Prod Group": item.prodgroup || "",
      Name: item.name || "",
      "Short Text For Code": item.shorttextforcode || "",
      Status: item.status || "",
      Action: item.action || "",
      Error: item.error || "",
    ...
    XLSX.utils.book_append_sheet(
      workbook,
      worksheet,
      "ReportedProblem Error Records"
    );

    // Generate filename with timestamp
    const timestamp = new Date().toISOString().slice(0, 19).replace(/:/g, "-");
    const filename = `reported_problem_errors_${exportType}_${timestamp}.xlsx`;
    ...
    // Add live update
    addLiveUpdate(
      `📥 Reported Problem error data exported: ${filename}`,
      "success"
    );
  };
