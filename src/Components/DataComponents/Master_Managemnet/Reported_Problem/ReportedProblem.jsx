...
function ReportedProblem() {
  ...
  const [totalReportedProblems, setTotalReportedProblems] = useState(0);
  ...
  const [availableCodeGroups, setAvailableCodeGroups] = useState([]);
  const [availableProdGroups, setAvailableProdGroups] = useState([]);
  ...
  const loadFilterOptions = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/collections/reportedproblem/filter-options`
      );
      const options = response.data;
      setAvailableCatalogs(options.catalogs || []);
      setAvailableCodeGroups(options.codeGroups || []);
      setAvailableProdGroups(options.prodGroups || []);
    } catch (error) {
  ...
  const applyFilters = async (pageNum = 1) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/collections/reportedproblem/filter?${filterParams}`
      );
      setData(response.data.reportedProblems || []);
      setTotalPages(response.data.totalPages || 1);
      setTotalReportedProblems(response.data.totalReportedProblems || 0);
      setLoader(false);
    } catch (error) {
  ...
  const downloadReportedProblemExcel = async () => {
    setIsDownloadingReportedProblem(true);
    try {
      ...
      if (response.ok) {
        ...
        toast.error("Failed to download Reported Problem Excel file");
      }
    } catch (error) {
      console.error("Error downloading file:", error);
      toast.error("Error downloading file");
    } finally {
      setIsDownloadingReportedProblem(false);
    }
  };
  ...
  const handleBulkDelete = () => {
    if (selectedRows.length === 0) {
      toast.error("Please select reported problems to delete");
      return;
    }

    Swal.fire({
      title: "Delete Selected Reported Problems?",
      text: `You are about to delete ${selectedRows.length} reported problems permanently!`,
      icon: "warning",
      ...
      if (result.isConfirmed) {
        axios
          .delete(
            `${process.env.REACT_APP_BASE_URL}/collections/reportedproblem/bulk`,
            {
          ...
          .catch((error) => {
            ...
            Swal.fire({
              ...
              text:
                ...
                "Failed to delete reported problems",
              icon: "error",
            });
          });
      }
  ...
  const handleSelectAll = () => {
    ...
    if (!selectAll) {
      setSelectedRows(data?.map((problem) => problem._id));
    } else {
      setSelectedRows([]);
    }
  };

  const handleRowSelect = (problemId) => {
    if (selectedRows.includes(problemId)) {
      setSelectedRows(selectedRows.filter((id) => id !== problemId));
    } else {
      setSelectedRows([...selectedRows, problemId]);
    }
  };
  ...
  const handleSearch = async (pageNum = 1) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/collections/searchreportedproblem?q=${searchQuery}&page=${pageNum}&limit=${limit}`
      );
      ...
      console.error("Error searching reported problems:", error);
      setData([]);
      setTotalPages(1);
      setTotalReportedProblems(0);
      setLoader(false);
    }
  };
  ...
  const getData = (pageNum = page) => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}/collections/reportedproblem?page=${pageNum}&limit=${limit}`
      )
      .then((res) => {
        setLoader(false);
        setData(res.data.reportedProblems);
        setTotalPages(res.data.totalPages);
        setTotalReportedProblems(res.data.totalReportedProblems || 0);
      })
      .catch((error) => {
        setLoader(false);
        console.log(error);
  ...
  const handleCreate = () => {
    axios
      .post(
        `${process.env.REACT_APP_BASE_URL}/collections/reportedproblem`,
        currentData
      )
      .then((res) => {
        getData();
          ...
          <div className="bg-gray-100 border border-gray-200 rounded-lg p-4 shadow-sm">
                ...
                <div className="relative flex-1">
                  <FormControl sx={{ flex: 1 }} size="sm">
                    <Input
                      ...
                      className="bg-gray-50 h-10 border border-gray-200 rounded-lg text-sm placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 w-full focus:bg-white transition-all duration-200"
                    />
                  </FormControl>
                </div>
            ...
            <div className="flex flex-wrap justify-between items-center gap-3 mb-4">
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={downloadReportedProblemExcel}
                  disabled={isDownloadingReportedProblem}
                  className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                    isDownloadingReportedProblem
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      {availableCodeGroups.map((codeGroup, index) => (
                        <option key={index} value={codeGroup}>
                          {codeGroup}
                        </option>
                      ))}
                    </select>
                  ...
                  <div>
                    ...
                    >
                      ...
                      {availableProdGroups.map((prodGroup, index) => (
                        <option key={index} value={prodGroup}>
                          {prodGroup}
                        </option>
                      ))}
                    </select>
          ...
          <div className="flex justify-between items-center">
            <div className="text-sm text-gray-600">
              {isSearchMode && searchQuery ? (
                <span>
                  Search Results:{" "}
                  <span className="font-semibold">{totalReportedProblems}</span>{" "}
                  reported problems found for "{searchQuery}"
                </span>
              ) : isFilterMode ? (
                <span>
                  Filtered Results:{" "}
                  <span className="font-semibold">{totalReportedProblems}</span>{" "}
                  reported problems found
                </span>
              ) : (
                <span>
                  Total Records:{" "}
            ...
            <ModalDialog size="lg" className="p-2 thin-scroll">
              <div className="flex items-start justify-between p-2 border-b px-5 border-solid border-blueGray-200 rounded-t thin-scroll">
                <h3 className="text-xl font-semibold">
                  {editModal
                    ? "Update ReportedProblem"
                    : "Create ReportedProblem"}
                </h3>
                <div
                  onClick={() => handleCloseModal()}
                  ...
                  <div className="grid md:grid-cols-2 md:gap-6 w-full">
                    <div className="relative w-full mb-5 group">
                      ...
                      <input
                        ...
                        required
                        onChange={(e) =>
                          handleFormData("catalog", e.target.value)
                        }
                        id="name"
                        value={currentData?.catalog}
                    ...
                    <div className="relative w-full mb-5 group">
                      ...
                      <input
                        type="text"
                        onChange={(e) =>
                          handleFormData("codegroup", e.target.value)
                        }
                        id="name"
                        value={currentData?.codegroup}
                        className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                      />
                    </div>
                    <div className="relative w-full mb-5 group">
                      ...
                      <input
                        type="text"
                        onChange={(e) =>
                          handleFormData("prodgroup", e.target.value)
                        }
                        id="name"
                    ...
                    <div className="relative w-full mb-5 group">
                      ...
                      <input
                        type="text"
                        onChange={(e) =>
                          handleFormData("shorttextforcode", e.target.value)
                        }
                        id="name"
                        value={currentData?.shorttextforcode}
                        className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                      />
                    </div>
                    <div>
