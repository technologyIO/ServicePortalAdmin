...
const UserData = () => {
  ...
  const loadFilterOptions = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/collections/equipment/filter-options`
      );
      const options = response.data;
      setAvailableDealers(options.dealers || []);
      setAvailableCustomers(options.customers || []);
  ...
  const applyFilters = async (pageNum = 1) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/collections/equipment/filter?${filterParams}`
      );
      setData(response.data.equipment || []);
      setTotalPages(response.data.totalPages || 1);
      setTotalEquipment(response.data.totalEquipment || 0);
      setLoader(false);
    } catch (error) {
  ...
  const downloadEquipmentExcel = async () => {
    ...
    try {
      let url = `${process.env.REACT_APP_BASE_URL}/excel/equipments/export-equipments`;
      ...
      if (response.ok) {
        ...
        toast.error("Failed to download Equipment Excel file");
      }
    } catch (error) {
  ...
  const handleSelectAll = () => {
    ...
    if (!selectAll) {
      setSelectedRows(data?.map((equipment) => equipment._id));
    } else {
      setSelectedRows([]);
    }
  };
  ...
  const handleBulkDelete = () => {
    if (selectedRows.length === 0) {
      toast.error("Please select equipment to delete");
      return;
    }

    Swal.fire({
      title: "Delete Selected Equipment?",
      text: `You are about to delete ${selectedRows.length} equipment permanently!`,
      icon: "warning",
  ...
  const handleRowSelect = (equipmentId) => {
    if (selectedRows.includes(equipmentId)) {
      setSelectedRows(selectedRows.filter((id) => id !== equipmentId));
    } else {
      setSelectedRows([...selectedRows, equipmentId]);
    }
  };
  ...
  const handleSearch = async (pageNum = 1) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/collections/searchequipment?q=${searchQuery}&page=${pageNum}&limit=${limit}`
      );
  ...
  const getData = (pageNum = page) => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}/collections/equipment?page=${pageNum}&limit=${limit}`
      )
      .then((res) => {
        setLoader(false);
        setData(res.data.equipment);
        setTotalPages(res.data.totalPages);
        setTotalEquipment(res.data.totalEquipment || 0);
      })
      .catch((error) => {
        setLoader(false);
        console.log(error);
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleFilterChange("dealer", e.target.value)
                      }
                      className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                    >
                  ...
                  <div>
                    ...
                    <select
                      value={filters.currentCustomer}
                      onChange={(e) =>
                        handleFilterChange("currentCustomer", e.target.value)
                      }
                      className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                    >
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={filters.palNumber}
                      onChange={(e) =>
                        handleFilterChange("palNumber", e.target.value)
                      }
                      className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                  ...
                  <div>
                    ...
                    <input
                      type="date"
                      value={filters.custWarrantyStartDate}
                      onChange={(e) =>
                        handleFilterChange(
                          "custWarrantyStartDate",
                          e.target.value
                  ...
                  <div>
                    ...
                    <input
                      type="date"
                      value={filters.custWarrantyEndDate}
                      onChange={(e) =>
                        handleFilterChange(
                          "custWarrantyEndDate",
                          e.target.value
            ...
            <div className="text-sm text-gray-600">
              {isSearchMode && searchQuery ? (
                <span>
                  ...
                  equipment found for "{searchQuery}"
                </span>
              ) : isFilterMode ? (
                <span>
                  Filtered Results:{" "}
