...
function ComplaintType() {
  ...
  const applyFilters = async (pageNum = 1) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/complaints/complainttype/filter?${filterParams}`
      );
  ...
  const downloadComplaintTypeExcel = async () => {
    ...
    try {
      let url = `${process.env.REACT_APP_BASE_URL}/excel/complainttypes/export-complainttypes`;
      ...
      if (response.ok) {
        ...
        toast.error("Failed to download Complaint Type Excel file");
      }
    } catch (error) {
  ...
  const handleBulkDelete = () => {
    if (selectedRows.length === 0) {
      toast.error("Please select complaint types to delete");
      return;
    }

    Swal.fire({
      title: "Delete Selected Complaint Types?",
      text: `You are about to delete ${selectedRows.length} complaint types permanently!`,
      icon: "warning",
      ...
      if (result.isConfirmed) {
        axios
          .delete(
            `${process.env.REACT_APP_BASE_URL}/complaints/complainttype/bulk`,
            {
          ...
          .catch((error) => {
            ...
            Swal.fire({
              ...
              text:
                ...
                "Failed to delete complaint types",
              icon: "error",
            });
          });
      }
  ...
  const handleSelectAll = () => {
    ...
    if (!selectAll) {
      setSelectedRows(data?.map((complaintType) => complaintType._id));
    } else {
      setSelectedRows([]);
    }
  };

  const handleRowSelect = (complaintTypeId) => {
    if (selectedRows.includes(complaintTypeId)) {
      setSelectedRows(selectedRows.filter((id) => id !== complaintTypeId));
    } else {
      setSelectedRows([...selectedRows, complaintTypeId]);
    }
  };
  ...
  const handleOpenModal = (complaintType) => {
    setCurrentData(complaintType);
    setEditModal(true);
    setShowModal(true);
  };
