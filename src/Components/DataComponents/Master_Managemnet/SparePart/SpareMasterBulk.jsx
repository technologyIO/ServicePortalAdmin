...
function SpareMasterBulk({ onClose }) {
  ...
  // Download function add karo - SpareMaster ke fields ke according
  const handleDownloadErrors = (exportType = "failed") => {
    ...
    // Prepare data for Excel - SpareMaster fields ke according
    const excelData = dataToExport.map((item) => ({
      ...
      "Rate (MRP)": item.rate || "",
      DP: item.dp || "",
      "Charges (Exchange Price)": item.charges || "",
      "Spare Image URL": item.spareiamegurl || "",
      Status: item.status || "",
      Action: item.action || "",
      Error: item.error || "",
    ...
    XLSX.utils.book_append_sheet(
      workbook,
      worksheet,
      "SpareMaster Error Records"
    );

    // Generate filename with timestamp
    const timestamp = new Date().toISOString().slice(0, 19).replace(/:/g, "-");
    const filename = `sparemaster_errors_${exportType}_${timestamp}.xlsx`;

    // Download file
    XLSX.writeFile(workbook, filename);

    // Add live update
    addLiveUpdate(`📥 SpareMaster error data exported: ${filename}`, "success");
  };
  ...
  const handleDownload = () => {
    ...
    // Add worksheet to workbook
    XLSX.utils.book_append_sheet(workbook, worksheet, "Spare Master Template");

    // Write and download file
    XLSX.writeFile(workbook, "spare_master_template.xlsx");
  };
