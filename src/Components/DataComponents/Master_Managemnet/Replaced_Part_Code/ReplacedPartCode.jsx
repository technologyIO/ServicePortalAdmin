...
function ReplacedPartCode() {
  ...
  const [isDownloadingReplacedPartCode, setIsDownloadingReplacedPartCode] = useState(false);
  ...
  const loadFilterOptions = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/collections/replacedpartcodes/filter-options`
      );
      const options = response.data;
      setAvailableCatalogs(options.catalogs || []);
      setAvailableCodeGroups(options.codeGroups || []);
      setAvailableCodes(options.codes || []);
    } catch (error) {
  ...
  const applyFilters = async (pageNum = 1) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/collections/replacedpartcodes/filter?${filterParams}`
      );
  ...
  const downloadReplacedPartCodeExcel = async () => {
    ...
    try {
      let url = `${process.env.REACT_APP_BASE_URL}/excel/replacedpartcodes/export-replacedpartcodes`;
      ...
      if (response.ok) {
        ...
        a.href = downloadUrl;
        a.download = `replaced_part_codes_data_${new Date().toISOString().split("T")[0]}.xlsx`;
        ...
        toast.error("Failed to download Replaced Part Code Excel file");
      }
    } catch (error) {
      console.error("Error downloading file:", error);
      toast.error("Error downloading file");
    } finally {
      setIsDownloadingReplacedPartCode(false);
    }
  };
  ...
  const handleSelectAll = () => {
    ...
    if (!selectAll) {
      setSelectedRows(data?.map((partCode) => partCode._id));
    } else {
      setSelectedRows([]);
    }
  };

  const handleRowSelect = (partCodeId) => {
    if (selectedRows.includes(partCodeId)) {
      setSelectedRows(selectedRows.filter((id) => id !== partCodeId));
    } else {
      setSelectedRows([...selectedRows, partCodeId]);
    }
  };
  ...
  const handleOpenModal = (partCode) => {
    setCurrentData(partCode);
    setEditModal(true);
    setShowModal(true);
  };
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          ...
          .then((res) => {
            Swal.fire("Deleted!", "Replaced part code has been deleted.", "success");
          })
          .then((res) => {
            getData();
          })
  ...
  const handleCreate = () => {
    axios
      .post(
        `${process.env.REACT_APP_BASE_URL}/collections/replacedpartcodes`,
        currentData
      )
      .then((res) => {
        getData();
          ...
          <div className="bg-gray-100 border border-gray-200 rounded-lg p-4 shadow-sm">
            ...
            <div className="flex flex-wrap justify-between items-center gap-3 mb-4">
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={downloadReplacedPartCodeExcel}
                  disabled={isDownloadingReplacedPartCode}
                  className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                    isDownloadingReplacedPartCode
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={filters.slno}
                      onChange={(e) => handleFilterChange('slno', e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
            ...
            <ModalDialog size="lg" className="p-2 thin-scroll">
              <div className="flex items-start justify-between p-2 border-b px-5 border-solid border-blueGray-200 rounded-t thin-scroll">
                <h3 className="text-xl font-semibold">
                  {editModal
                    ? "Update ReplacedPartCode"
                    : "Create ReplacedPartCode"}
                </h3>
                <div
                  onClick={() => handleCloseModal()}
                ...
                <div className="w-[300px] md:w-[500px] lg:w-[700px] border-b border-solid border-blueGray-200 p-3 flex-auto max-h-[380px] overflow-y-auto">
                  <div className="grid md:grid-cols-2 md:gap-6 w-full">
                    ...
                    <div className="relative w-full mb-5 group">
                      ...
                      <input
                        type="text"
                        onChange={(e) => handleFormData("code", e.target.value)}
                        id="name"
                    ...
                    <div className="relative w-full mb-5 group">
                      ...
                      <input
                        type="text"
                        onChange={(e) => handleFormData("slno", e.target.value)}
                        id="name"
                        value={currentData?.slno}
                        className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px] focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                      />
                    </div>
                    <div>
...
export default ReplacedPartCode;
