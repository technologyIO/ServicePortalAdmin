...
function ReplacePartCodeBulk({ isOpen, onClose, getData }) {
  ...
  // Download function add karo - ReplacePartCode ke fields ke according
  const handleDownloadErrors = (exportType = "failed") => {
    ...
    // Prepare data for Excel - ReplacePartCode fields ke according
    ...
    XLSX.utils.book_append_sheet(
      workbook,
      worksheet,
      "ReplacePartCode Error Records"
    );

    // Generate filename with timestamp
    const timestamp = new Date().toISOString().slice(0, 19).replace(/:/g, "-");
    const filename = `replace_part_code_errors_${exportType}_${timestamp}.xlsx`;
    ...
    // Add live update
    addLiveUpdate(
      `📥 Replace Part Code error data exported: ${filename}`,
      "success"
    );
  };
